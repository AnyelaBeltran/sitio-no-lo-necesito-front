<template>
  <h1 class="display-4 text-center">Editar Consola</h1>

  <div class="row">
    <div class="col col-md-6 offset-md-3 ">

      <form @submit.prevent="actualizarConsola" enctype="multipart/form-data">
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await axios.get(`/dashboard-consolas/${id}`, {
        headers: {
          'auth-token': localStorage.getItem('token')
        }
      });

      this.nuevaConsola.nombre = response.data.data.nombre;
      this.nuevaConsola.precio = response.data.data.precio;
      this.nuevaConsola.stock = response.data.data.stock;
      this.nuevaConsola.marca_id = response.data.data.marca_id;
      this.nuevaConsola.descripcion = response.data.data.descripcion;
      this.nuevaConsola.estado = response.data.data.estado;

    } catch (error) {
      console.error('Error al obtener los datos de la consola:', error);
      Swal.fire({
        icon: 'error',
        title: 'Error al obtener los datos de la consola',
        text: 'Ha ocurrido un error al intentar obtener los datos de la consola. Por favor, inténtalo nuevamente.',
      });
    }
  },
  ...
  methods: {
    ...
    async actualizarConsola() {
      ...
      console.log(this.nuevaConsola.estado);

      try {
        const response = await axios.put(`/dashboard-consolas/${id}`, formData, {
          headers: {
            'auth-token': localStorage.getItem('token')
          },
        ...
        // Aquí puedes manejar la respuesta exitosa si es necesario, por ejemplo, mostrar un mensaje de éxito
        Swal.fire({
          icon: 'success',
